...
1. **What is this sequence for?** The service, the offer, or the moment in the job.
   Examples: a new service enquiry, a quote that has not been answered, a finished
...
Write the way you talk to a customer during a service consultation. Concrete beats clever.
...
## Step 5: Provision in GHL

The workflow builder has **no public API**. See the capability matrix in
`../ghl-crm/references/capability-matrix.md`.

Templates can use any of the three API access lanes:

- **MCP, optional**: run `claude mcp list` and use the registered GoHighLevel server name.
- **Direct REST**: use `POST /emails/builder` and `/emails/builder/data` with the token
  from repo-root `secrets/ghl.env`. `updatedBy` is required.
- **CLI**: use repo-root `scripts/ghl raw` for the same endpoints.

Open every returned `previewUrl`. The full recipe is in `ghl-email-flows`.

The workflow itself has no public API. Use `ghl-browser`.

1. Create a workflow named `<purpose>-nurture-<yyyy-mm-dd>`
2. Set the trigger from question 2 of the brief
3. Add five email actions with the delays from the table, attaching the API templates
...
- **Unsubscribe link and the business's legal postal address in every email.** Follow the
  commercial email laws that apply to the business and its recipients
