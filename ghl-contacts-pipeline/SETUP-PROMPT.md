...
```text
Set up and verify the ghl-contacts-pipeline skill.

1. Confirm ~/.claude/skills/ghl-contacts-pipeline/SKILL.md and the ghl-crm skill exist.
...
4. Without MCP, use direct REST with the Private Integration Token or the repo-root `scripts/ghl` command.
5. Run a read-only contact-list query. On failure, ask Claude to retry after checking the token scope and location ID.
```
