...
Paste into Claude Code to install + verify the ghl-payments-invoicing skill.

```
Install + verify the ghl-payments-invoicing skill on this machine.

1. Confirm the skill exists at ~/.claude/skills/ghl-payments-invoicing/ (symlink into the kit's GHL connector pack).
...
4. Dry test: load the ghl-payments-invoicing skill and run invoices, estimates, products, coupons, payment reads with a read-only call
   (any playbook step 1). Expect real data back, no 401.
5. Ready. Trigger phrases include "create an invoice", "list transactions", "set up a recurring invoice".
```
